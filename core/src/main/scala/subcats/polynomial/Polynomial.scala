...
object Polynomial {
  ...
  /**
    * In essence: {{{
    *   sealed abstract class TypeF protected() { type F[A] }
    *   object TypeF {
    *     sealed abstract class Aux[F_[_]] extends TypeF { type F[A] = F_[A] }
    *     final class I extends Aux[λ[X => X]]
    *     final class K[A] extends Aux[λ[X => A]]
    *     final class :+:[A <: TypeF, B <: TypeF] extends Aux[λ[X => Either[A#F[X], B#F[X]]]]
    *     final class :*:[A <: TypeF, B <: TypeF] extends Aux[λ[X => (A#F[X], B#F[X])]]
    *   }
    *   import TypeF._
